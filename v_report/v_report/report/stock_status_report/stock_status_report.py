...
def get_ordered_items(filters, set_items):
	...
	ordered_items = frappe.db.sql("""
		...
		where 
			...
			and so.company=%(company)s and so.foreign_buyer_name =%(buyer)s and so_item.parent=so.name and so.docstatus=1 
		group by 
			so_item.item_code""",
...
def dispatched_item_report(filters):
	...
	for i in dispatch_items:
		...
		data.append([1, i.item_code, i.item_name, s_qty, s_uom, s_op_stock, s_production_qty, s_ordered_qty, s_delivered_qty, s_pending_qty, s_closing_stock, s_remain_qty])
...
def set_item_report(filters):
	...
	ordered_items_map = get_ordered_set_items(filters)	
	for i in items:
		if i.is_stock_item:
